...
使用 langchain-mcp-adapters 连接远程 MCP 服务器并获取工具。
通过 Tool Interceptors 注入 user_id 等上下文信息。
"""
...
class MCPToolService:
    ...
    def _build_server_configs(cls) -> dict:
        ...
        for server in servers:
            ...
            log(f"配置 MCP 服务器: {server_name} -> {server_url}", "MCP")
    ...
    async def _inject_user_context(cls, request: MCPToolCallRequest, handler):
        ...
        if user_id:
            log(f"MCP Interceptor: 注入 user_id={user_id}", "MCP")
    ...
    def _init_client(cls):
        ...
        try:
            ...
            log(f"MCP 客户端初始化完成，共 {len(server_configs)} 个服务器", "MCP")
        except Exception as e:
            log(f"MCP 客户端初始化失败: {e}", "MCP")
            cls._client = None
    ...
    async def get_tools_async(cls) -> List[BaseTool]:
        ...
        try:
            ...
            log(f"获取到 {len(wrapped_tools)} 个 MCP 工具", "MCP")
            cls._tools_cache = wrapped_tools
            return wrapped_tools
        except Exception as e:
            log(f"获取 MCP 工具失败: {type(e).__name__}: {e}", "MCP")
            return []
